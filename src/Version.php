...
/**
 * @property Model|\Visualbuilder\Versionable\Versionable $versionable
...
class Version extends Model
{
    ...
    public static function createForModel(Model $model, array $replacements = [], $time = null): Version
    {
        ...
        // Set polymorphic user relationship
        $versionUser = $model->getVersionUser();
        if ($versionUser) {
            $version->user_id = $versionUser->getKey();
            $version->user_type = $versionUser->getMorphClass();
        }
    ...
    public function revertWithoutSaving(): ?Model
    {
        ...
        switch ($this->versionable->getVersionStrategy()) {
            ...
            case VersionStrategy::SNAPSHOT:
                // v1 + vN
                /** @var \Visualbuilder\Versionable\Version $initVersion */
